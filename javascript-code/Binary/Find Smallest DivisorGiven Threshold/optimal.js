...
function findSmallestDivisor(arr, limit) {
  ...
  //Note;testing divisors beyond max(arr) gives no new results. it's rule
  ...
  while (low <= high) {
    ...
    if (sum <= limit) {//If sum ≤ limit → current divisor might be valid → try to find smaller divisor → high = mid - 1.
      //samllest divisor we get on left side and larger sum we aslo get on left side 
      //If sum > limit → divisor too small → increase it → low = mid + 1. for this go right 
